...
|------------|-----------|-----------------------|---------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `--add`    | `-a`      | `--command` `--scope` | `--force`           | Adds a new alias for a <br>`--command <command>`<br>to the pls file at a given<br>`--scope <[g]lobal, [l]ocal, [h]ere>`.<br> Use with `--force` to overwrite the alias if it exists.                    |
...
| `--scope`       |yes              | `--add` `--delete`  `--list` | The selected scope for `--add`, `--delete`, and `--list` operations. Allowed scopes are [g]lobal, [l]ocal, [a]ll, [h]ere.  The 'all' scope is exclusive to `--list`. The 'here' scope is excluded from `--list`.     |
| `--print`       |               | root command `--list`        | Prints the command instead of executing it when used with root command.  Useful for piping to other commands or shells. Or, if used in conjunction with `--list`, it additionally prints the command for each alias. |
